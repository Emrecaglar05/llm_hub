...
"""
Bu Python kodu, OpenAI’nin Function Calling özelliğini kullanarak çalışan bir chatbot sistemi oluşturuyor. Kod, kullanıcıların doğal dildeki sorularını anlayıp, gerektiğinde önceden tanımlanmış fonksiyonları çağırarak yanıt üretmesini sağlıyor. Temel olarak şu şekilde çalışıyor:

Sınıf Yapısı: FunctionCallingChatbot sınıfı, chatbotun tüm konuşma geçmişini (conversation_history) ve kullanılabilir fonksiyonları (available_functions) yönetiyor.

Fonksiyonlar: Chatbot, matematiksel hesaplamalar (dikdörtgen, daire, üçgen alanı), hava durumu sorgulama, döviz dönüştürme, geçerli zamanı alma ve e-posta doğrulama gibi fonksiyonları çağırabiliyor. Bu fonksiyonlar demo verileriyle çalışıyor ve kullanıcıya bilgi sağlıyor.

Function Calling: Kullanıcı mesajı alındığında, AI modeline (gpt-3.5-turbo) gönderiliyor. Model, gerekirse function_call ile uygun fonksiyonu çağırıyor. Fonksiyonun çıktısı tekrar modele gönderiliyor ve AI, sonucu yorumlayarak kullanıcıya anlaşılır şekilde sunuyor.

Konuşma Yönetimi: Chatbot, önceki mesajları ve yapılan fonksiyon çağrılarını hatırlayarak bir konuşma akışı oluşturuyor. Bu sayede devam eden diyaloglarda bağlam korunuyor.

Demo ve İnteraktif Mod: Kodun sonunda otomatik bir demo çalıştırılıyor; belirli sorular sorularak AI yanıtları gösteriliyor. Ayrıca main() fonksiyonu ile kullanıcı, interaktif olarak chatbot ile etkileşime geçebiliyor ve istediği kadar soru sorabiliyor.

Özet ve İstatistik: Konuşma sırasında yapılan toplam mesaj sayısı ve fonksiyon çağrıları takip ediliyor, kullanıcıya özet olarak sunulabiliyor.
