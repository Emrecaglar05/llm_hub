...
load_dotenv()
client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"))  # API anahtarını .env dosyasından al

def basic_chat_completion(): # Basit kullanıcı sorusunu yanıtlama
    """Temel ChatCompletion kullanımı"""
    
    response = client.chat.completions.create( 
        model="gpt-3.5-turbo",
        messages=[
            {"role": "user", "content": "Python'da liste ve tuple arasındaki fark nedir?"} # kullanıcı mesajı
        ]
    )
    
    return response.choices[0].message.content # AI mesajı döndürülür



def chat_with_system_message():  # System mesajı ile davranış belirleme rol atama.
    """System message ile davranış belirleme"""
    
    messages = [
        {
            "role": "system", 
            "content": "Sen bir Python öğretmeni olan yardımcısın. Açıklamalarını basit ve kod örnekleriyle destekle." # sistem mesajı
        },
        {
            "role": "user", 
            "content": "Decorators nasıl çalışır?" # kullanıcı mesajı
        }
    ]
    
    response = client.chat.completions.create(
        model="gpt-3.5-turbo",# model seçimi
        messages=messages, # mesaj dizisi
...
def conversation_management(): # KOnuşma geçmişi yönetimi
    """Konuşma geçmişi yönetimi"""
    
    conversation = [ # konuşma geçmişi
        {"role": "system", "content": "Sen yardımcı bir AI asistanısın."}, # sistem mesajı
        {"role": "user", "content": "Merhaba! Python öğrenmeye yeni başladım."}, # kullanıcı mesajı
    ]
    
    # İlk yanıt
    response1 = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=conversation # önceki konuşma geçmişiyle birlikte 
    )
...
def multiple_choices_example():
    ...
    choices = []
    for i, choice in enumerate(response.choices): # her bir yanıtı listeye ekle
        choices.append({
            "choice_number": i + 1, # seçenek numarası
            "content": choice.message.content # yanıt içeriği
        })
